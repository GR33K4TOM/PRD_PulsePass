...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("SELECT DISTINCT e FROM Event e JOIN e.artists a JOIN e.venue v WHERE e.status = 'PUBLISHED' AND e.eventDate > :date AND v.city = :city AND LOWER(a.stageName) LIKE LOWER(CONCAT('%', :artistText, '%')) ORDER BY e.eventDate ASC")
    List<Event> findRecommendedEvents(@Param("date") LocalDateTime date, @Param("city") String city, @Param("artistText") String artistText);
}
